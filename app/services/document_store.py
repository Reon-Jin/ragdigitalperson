...
class DocumentStore:
    def __init__(self, settings: Settings) -> None:
        ...
        self.chunk_vectorizer = self._make_vectorizer()
    ...
    def _reindex_existing_uploads(self) -> None:
        legacy_docs = list(self.docs)
        self.docs = []
        self.sections = []
        self.chunks = []
        self.pages = []
        ...
        for doc in legacy_docs:
            ...
            original_name = doc.get("filename", f"{doc_id}{suffix}")
            if not doc_id:
                continue
            stored_path = self.settings.uploads_dir / f"{doc_id}{suffix}"
            if not stored_path.exists():
                continue
            prepared = self.prepare_document(stored_path, original_name, preserve_doc_id=doc_id)
            if prepared:
                self._upsert_prepared_document(prepared)
    ...
    def _rebuild_index(self) -> None:
        self.doc_matrix = self.doc_vectorizer.fit_transform([self._doc_search_text(doc) for doc in self.docs]) if self.docs else None
        self.section_matrix = (
            self.section_vectorizer.fit_transform([self._section_search_text(section) for section in self.sections])
            if self.sections
            else None
        )
        self.chunk_matrix = (
            self.chunk_vectorizer.fit_transform([self._chunk_search_text(chunk) for chunk in self.chunks])
            if self.chunks
            else None
        )
    ...
    def _upsert_prepared_document(self, prepared: Dict[str, Any]) -> None:
    ...
    def update_chunk_title(self, doc_id: str, chunk_id: str, chunk_title: str) -> Dict[str, Any] | None:
        ...
        return {
            ...
            "chunk_title": updated["chunk_title"],
            "section_id": updated["section_id"],
            "section_title": updated["section_title"],
    ...
    def add_files(self, file_paths: Sequence[Path]) -> Dict[str, List]:
        ...
        for temp_path in file_paths:
            ...
            destination = self.settings.uploads_dir / stored_name
    ...
    def _score_candidates(
        ...
        if not items or matrix is None:
            return {}
        ...
        for query in valid_queries:
            ...
            query_scores = cosine_similarity(query_matrix, matrix).flatten()
            for index, item in enumerate(items):
                identifier = item[identifier_key]
                scores[identifier] = max(scores.get(identifier, 0.0), float(query_scores[index]))
        return scores

    def _subset_matrix(self, matrix, indexes: Sequence[int]):
        if matrix is None or not indexes:
            return None
    ...
    def rank_chunks(
        ...
        *,
        ...
        matrix = self._subset_matrix(self.chunk_matrix, [self.chunk_index_by_id[chunk["chunk_id"]] for chunk in candidates])
        scores = self._score_candidates(queries, candidates, self.chunk_vectorizer, matrix, "chunk_id")
        ranked = sorted(candidates, key=lambda item: scores.get(item["chunk_id"], 0.0), reverse=True)

        results: List[SearchResult] = []
        for chunk in ranked[:limit]:
            score = round(scores.get(chunk["chunk_id"], 0.0), 4)
